...
import { updateLead } from '../../services/service';
...
export default function EditLeadScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.source) {
      // Should not happen for edit, but safe to check
    ...
    setLoading(true);
    try {
      // Clean payload to match web implementation
      const payload = {
      ...
      const msg = error?.response?.data?.detail || 'Failed to update lead';
      Toast.error(msg);
    } finally {
      setLoading(false);
    }
            ...
            <View style={styles.formCard}>
              ...
              >
                {loading ? (
                  ...
                  <Text style={styles.submitBtnText}>Update Lead</Text>
                )}
              </TouchableOpacity>
            </View>
          </ScrollView>
